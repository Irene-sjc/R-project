...
# or inside the Shiny app folder.

possible_files <- c(
  "PLACES_sample_shinyapp.csv",
  "Shiny app/PLACES_sample_shinyapp.csv"
)

data_file <- possible_files[file.exists(possible_files)][1]

if (is.na(data_file)) {
  stop("Data file not found. Please make sure PLACES_sample_shinyapp.csv is in the Shiny app folder.")
}
